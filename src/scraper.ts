...
async function decryptEmbed(html: string) {
  ...
  if (!container) throw new Error(`containerName not found`);
  const arr = container[fields.arrayName];
  if (!arr) throw new Error(`arrayName not found`);
  const obj = arr[0][fields.objectName];
  if (!obj) throw new Error(`objectName not found`);
  const frag1 = b64toU8(obj[fields.keyField]);
  const iv = b64toU8(obj[fields.ivField]);
  const kf2raw = data[fields.keyFrag2Field];
  if (!kf2raw) throw new Error('key fragment 2 missing');
  ...
  if (!url.startsWith('http')) throw new Error(`Unexpected decrypted value`);
...
async function searchReanime(query: string): Promise<any[]> {
  console.log('[REANIME] Searching for:', query);
  const res = await fetch(`${BASE}/api/v1/search?${new URLSearchParams({ q: query, limit: '10' })}`, { headers: H });
  console.log('[REANIME] Search response status:', res.status);
  if (!res.ok) throw new Error(`reanime search ${res.status}`);
  const data: any = await res.json();
  console.log('[REANIME] Search results:', data?.results?.length || 0);
...
async function resolveSeries(anilistId: number | string) {
  console.log('[REANIME] Resolving series for AniList ID:', anilistId);
  const cacheKey = `reanime:series:${anilistId}`;
  const cached = cacheGet<any>(cacheKey);
  if (cached) {
    console.log('[REANIME] Using cached series data');
    return cached;
  }

  const media = await getMedia(anilistId);
  console.log('[REANIME] Got AniList media:', media?.title?.english);
  const malId = media.idMal ?? null;
  const queries = buildTitles(media).slice(0, 5);
  console.log('[REANIME] Search queries:', queries);
  ...
  console.log('[REANIME] Found', candidates.size, 'candidates');
  ...
  for (const [id, r] of candidates) {
    ...
    if (coverId && coverId === Number(anilistId)) {
      ...
      console.log('[REANIME] Matched via cover image');
  ...
  for (const { id, detail } of details) {
    if (detail?.anilist_id && Number(detail.anilist_id) === Number(anilistId)) {
      ...
      console.log('[REANIME] Matched via anilist_id');
      cacheSet(cacheKey, data, 3600);
      return data;
    }
  }
  ...
  if (malId) {
    for (const { id, detail } of details) {
      if (detail?.mal_id && Number(detail.mal_id) === Number(malId)) {
        ...
        console.log('[REANIME] Matched via mal_id');
        cacheSet(cacheKey, data, 3600);
        return data;
      }
    }
...
export async function getReanimeEpisodes(anilistId: number | string) {
  console.log('[REANIME] Getting episodes for:', anilistId);
  const series = await resolveSeries(anilistId);
  console.log('[REANIME] Series resolved:', series.animeId);
  const reanimeEps = await fetchEpisodesList(series.animeId);
  console.log('[REANIME] Found', reanimeEps.length, 'episodes');
  
  if (!reanimeEps.length) throw new Error(`ReAnime: no episodes found`);
...
export async function getReanimeWatch(anilistId: number | string, audio: 'sub' | 'dub', epNum: number) {
  console.log('[REANIME] Getting watch for ep', epNum, audio);
  const series = await resolveSeries(anilistId);
  ...
  if (!servers.length) throw new Error(`ReAnime: no ${audio} servers`);

  console.log('[REANIME] Fetching embed from:', servers[0].dataLink);
  const embedRes = await fetch(servers[0].dataLink, { headers: { ...H, Referer: `${BASE}/` } });
  if (!embedRes.ok) throw new Error(`ReAnime: embed fetch failed`);
  const stream = await decryptEmbed(await embedRes.text());
  console.log('[REANIME] Stream decrypted successfully');
